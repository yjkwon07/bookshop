package com.yjkwon.bookshop.common.log;
...
public class LoggingAdvice {
	...
	@Before("execution(* com.yjkwon.bookshop.*.service.*.*(..)) or "
			+ "execution(* com.yjkwon.bookshop.*.dao.*.*(..))")
	public void startLog(JoinPoint jp) {
		...
		logger.info("req(모든 파라미터들):" + Arrays.toString(jp.getArgs()));
	...
	@After("execution(* com.yjkwon.bookshop.*.service.*.*(..)) or "
			+ "execution(* com.yjkwon.bookshop.*.*.dao.*.*(..))")
	...
	@Around("execution(* com.yjkwon.bookshop.*.service.*.*(..)) or "
			+ "execution(* com.yjkwon.bookshop.*.dao.*.*(..))")
	public Object timeLog(ProceedingJoinPoint pjp) throws Throwable {
		long startTime = System.currentTimeMillis();
		logger.info("==============================");
		logger.info("target 메소드의 동작 시간");
